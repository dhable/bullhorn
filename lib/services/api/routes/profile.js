...
exports.init = function(server, log, conf) {
   ...
   server.post({path: "/applications/:appId/recipients", version: "1.0.0"},
               common.authorize, common.loadDomainFromAccessKey, common.loadApplication, _.partial(createRecipient, log), returnRecipient);

   server.get({path: "/applications/:appId/recipients", version: "1.0.0"},
              common.authorize, common.loadDomainFromAccessKey, common.loadApplication, _.partial(loadAllRecipients, log), returnAllRecipients);

   server.get({path: "/applications/:appId/recipients/:recipientId", version: "1.0.0"},
              common.authorize, common.loadDomainFromAccessKey, common.loadApplication, _.partial(loadRecipient, log), returnRecipient);

   server.put({path: "/applications/:appId/recipients/:recipientId", version: "1.0.0"},
              common.authorize, common.loadDomainFromAccessKey, common.loadApplication, _.partial(loadRecipient, log), _.partial(updateRecipient, log), returnRecipient);

   server.del({path: "/applications/:appId/recipients/:recipientId", version: "1.0.0"},
              common.authorize, common.loadDomainFromAccessKey, common.loadApplication, _.partial(deleteRecipient, log), common.emptyOKChainHandler);
};
