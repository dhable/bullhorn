...
/**
 * Stat collection bucket for the Email Drain.
 */
var statsCollector = exports.stats = new stats.Collector();
...
exports.pour = function(to, message, template, callback) {
  ...
  client.send(email, function(err, resp) {
    ...
    if(err) {
      statsCollector.record({type: stats.Type.Count, error: 1},
                            {type: stats.Type.Midpoint, time: duration});
      return callback(err);
    }

    statsCollector.record({type: stats.Type.Count, message: 1},
                          {type: stats.Type.Midpoint, time: duration});
