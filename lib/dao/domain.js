...
var domainFields = ["name", "env", "externalId", "numRecipients", "numMessages", "channels", "recipients"],
   ...
   return new rsvp.Promise(function(accept, reject) {
      firebase.child(domainId).on("value", function(resultSet) {
         ...
         if(domainObj) {
            ...
            domainObj.recipients = common.dbDeserializeArray(domainObj.recipients);
...
var save = function(firebase, id, domainObj, accept, reject) {
   ...
   domainCopy.recipients = common.dbSerializeArray(domainCopy.recipients);
