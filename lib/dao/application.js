...
var applicationFields = ["name", "env", "externalId", "numRecipients", "numMessages", "channels", "recipients"],
   ...
   return new rsvp.Promise(function(accept, reject) {
      firebase.child(appId).on("value", function(resultSet) {
         ...
         if(application) {
            ...
            application.recipients = common.dbDeserializeArray(application.recipients);
            application.channels = common.dbDeserializeArray(application.channels);
...
var save = function(firebase, id, application, accept, reject) {
   ...
   var appCopy = _.extend(application);
   appCopy.recipients = common.dbSerializeArray(appCopy.recipients);
   appCopy.channels = common.dbSerializeArray(appCopy.channels);

   firebase.child(id).set(appCopy, function(err) {
      if(!err) {
...
var update = function(firebase, application) {
   return new rsvp.Promise(function(accept, reject) {
      if(!application.id) {
