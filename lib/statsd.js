...
var client = new statsd.StatsD({
  ...
  prefix: conf().get("statsd.namespace") + ".",
  suffix: "",
  globalize: false,
 ...
 * @param numRecipients The total number of recipients that potentially could have recieved the notification.
 ...
 */
exports.recordNotificationProcessed = function(drainName, numRecipients, numSuccess, numFailure, processingTime) {
  client.increment(drainName + ".processed");
  client.increment(drainName + ".recipients", numRecipients);
  client.increment(drainName + ".success", numSuccess);
  client.increment(drainName + ".failure", numFailure);
  client.gauge(drainName + ".execution-time", processingTime);
};
